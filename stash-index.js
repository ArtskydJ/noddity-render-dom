...
module.exports = function renderDom(rootPostOrString, options, cb) {
	...
	postOrString(rootPostOrString, butler, function (err, rootPost) {
		...
		var onPostChange = makePostChangeHandler(rendered, {
...
function makePostChangeHandler(intialRender, util) {
	return function (_, post) {
		var partialString = makePartialString(post.filename)
		util.ractive.resetPartial('current', partialString)
		scan(post, util, intialRender.filenameUuidsMap, intialRender.uuidArgumentsMap)
	}
}
